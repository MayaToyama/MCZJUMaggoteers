...
java -version
if [[ ! -f "$HOME/.m2/repository/io/mczju/MCZJUItemCreator/1.1.0/MCZJUItemCreator-1.1.0.jar" ]]; then
  echo "Installing MCZJUItemCreator 1.1.0 to local repo..."
  (cd "$IC" && "$MVN" install -DskipTests -q)
fi
cd "$PLUGIN"
"$MVN" package -q
echo "Built: $PLUGIN/target/Maggoteers-0.1.0-SNAPSHOT.jar"
