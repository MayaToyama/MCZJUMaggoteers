...
- 删除「全员倒下时传送到当前刷怪点上方 40 格俯瞰」逻辑：`noneAlive` 判断保留（仍用于 `markDefeated`），仅移除 `teleport(spawn + 40)` 传送块。

**决策与原因**
- 用户反馈失败变观察者会被传送到很高的地方，观感突兀：40 格过高，且锚点是**刷怪点**而非死亡点，会横向瞬移一大段。
- 观察者本就自由飞行，无需俯视锚定；留在倒下位置由玩家自行调整视角即可。此条取代下方「旁观者夜视修复」中"全灭时才统一传送到当前层出生点上方俯瞰"的描述。
